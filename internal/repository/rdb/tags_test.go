...
func testTagToManyLinksTags(t *testing.T) {
	...
	if err = randomize.Struct(seed, &b, linksTagDBTypes, false, linksTagColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, linksTagDBTypes, false, linksTagColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.LinksTags().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	a.R.LinksTags = nil
	if err = a.L.LoadLinksTags(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.LinksTags); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testTagToManyAddOpLinksTags(t *testing.T) {
	...
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, linksTagDBTypes, false, strmangle.SetComplement(linksTagPrimaryKeyColumns, linksTagColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*LinksTag{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddLinksTags(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.LinksTags[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.LinksTags[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.LinksTags().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
